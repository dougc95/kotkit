...
// app's own tsconfig — under `exactOptionalPropertyTypes` — before any route
// handler depends on it. The `@ts-expect-error` proves the excess-property
// check actually fires for a caller-supplied `realm` (identity-realm:
// "Client cannot choose the realm" — every user-owned record's realm is
// stamped by the identity plugin server-side, never read off the wire). No
// runtime code: this file is never imported.
